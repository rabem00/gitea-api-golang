...
func createUser(client *gitea.Client, name string, email string) {
	...
	*change = false
	...
	fmt.Println(usr.UserName + " created.")
}

func deleteUser(client *gitea.Client, name string) {
	err := client.AdminDeleteUser(name)
	if err != nil {
		fmt.Println(err)
		return
	}
...
func printUsage() {
	...
	fmt.Println("deleteuser\t\t- to delete an user from gitea")
...
func main() {
	...
	// Flag set delete an user from gitea
	deleteuser := flag.NewFlagSet("deleteuser", flag.ExitOnError)
	deleteUserFlag := deleteuser.String("l", "", "Login name of the user")
	...
	case "deleteuser":
		deleteuser.Parse(os.Args[2:])
		if *deleteUserFlag != "" {
			deleteUser(client, *deleteUserFlag)
